...
// ==================== LISTAGEM ====================
export const getInstitutionalContracts = async (
  params: paramsGetInstitutionalContracts,
) => {
  const { data } =
    await axiosNestFinance.get<GetInstitutionalContractsResponse>(
...
export const getInstitutionalContractAlertas = async () => {
  const { data } =
    await axiosNestFinance.get<GetInstitutionalContractAlertasResponse>(
...
//Tipagem
export interface ToggleContractEstadoResponse {
  success: boolean;
  message: string;
  data: {
    codigoContrato: number;
...
export async function toggleContractEstado(
  id: string,
): Promise<ToggleContractEstadoResponse> {
  const { data } = await axiosNestFinance.patch<ToggleContractEstadoResponse>(
    `/institutional-contract/${id}/estado`,
  );
  return data;
}

export async function deleteInstitutionalContract(id: string) {
  const { data } = await axiosNestFinance.delete(
    `/institutional-contract/${id}`,
  );
  return data;
}
