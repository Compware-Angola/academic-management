...
export default function AtribuirDescontoModal({
  ...
  const [observacao, setObservacao] = useState<string>(initial?.observacao ?? "");
  const [codigoMatriculaStr, setCodigoMatriculaStr] = useState<string>(
    initial?.codigoMatricula ? String(initial.codigoMatricula) : ""
  );
  const [codigoTaxa, setCodigoTaxa] = useState<string>(initial?.codigoTaxa ? String(initial.codigoTaxa) : "");
  const [codigoInstituicao, setCodigoInstituicao] = useState<string>(initial?.codigoInstituicao ? String(initial.codigoInstituicao) : "");
  const [codigoAnoStr, setCodigoAnoStr] = useState<string>(initial?.codigoAno ? String(initial.codigoAno) : "");
  const [semestre, setSemestre] = useState<string>(initial?.semestre ? String(initial?.semestre) : "");

  const [pesquisar, setPesquisar] = useState(false);
  const [modalAberto, setModalAberto] = useState(false);
  const [alunoConfirmado, setAlunoConfirmado] = useState(false);

  const { data: aluno, isLoading: isLoadingAluno, isError, error } = useQueryAlunoMatricula(codigoMatriculaStr, pesquisar);

  useEffect(() => {
    if (open) {
      setObservacao(initial?.observacao ?? "");
      setCodigoMatriculaStr(initial?.codigoMatricula ? String(initial.codigoMatricula) : "");
      setCodigoTaxa(initial?.codigoTaxa ? String(initial.codigoTaxa) : "");
      setCodigoInstituicao(initial?.codigoInstituicao ? String(initial.codigoMatricula) : "");
  ...
  return (
    <Dialog open={open} onOpenChange={handleOnOpenChange}>
      <DialogContent>
          <DialogHeader>
              <DialogTitle>{isEditing ? "Editar Atribuição" : "Atribuir Desconto"}</DialogTitle>
              <DialogDescription>
                  {isEditing ? "Altere os dados da atribuição abaixo." : "Atribuir um desconto ao estudante."}
              </DialogDescription>
          </DialogHeader>
        ...
        <div className="grid grid-cols-1 gap-4">
          <div className="space-y-2">
            ...
            <div className="flex gap-2">
              ...
              />
              <Button type="button" size="icon" disabled={isLoadingAluno || !codigoMatriculaStr} onClick={pesquisarAluno}>
                {isLoadingAluno ? "..." : <Search className="h-4 w-4" />}
              </Button>
            </div>
            {isError && (
              <p className="text-sm text-red-500 font-medium">{(error as unknown as AxiosError<{ message?: string }>)?.response?.data?.message ?? "Estudante não encontrado"}</p>
            )}
            {alunoConfirmado && aluno && (
              <p className="text-sm text-green-600 font-medium">Aluno: {aluno.nome_completo}</p>
            )}
          </div>

            <DescontoSelect value={codigoTaxa} onChangeValue={(v) => setCodigoTaxa(v)} />
        ...
        <div className="space-y-2">
          ...
          <div>
            <AcademicYearSelect value={codigoAnoStr} onChangeValue={(v) => setCodigoAnoStr(v)} />
          </div>
          ...
          <div>
            ...
            <Input id="observacao" value={observacao} onChange={(e) => setObservacao(e.target.value)} disabled={isSubmitting} />
          </div>
        </div>
        ...
        <DialogFooter>
          <Button variant="outline" onClick={() => onOpenChange(false)} disabled={isSubmitting}>
            ...
            <Button onClick={handleSubmit} disabled={isSubmitting}>
                {isSubmitting
                    ? (isEditing ? "Salvando..." : "Atribuindo...")
                    : (isEditing ? "Salvar Alterações" : "Atribuir Desconto")}
            </Button>
        </DialogFooter>
      </DialogContent>
      <ConfirmarAlunoModal
