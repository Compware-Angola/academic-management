...
function StatusBadge({ dataFim }: { dataFim: string }) {
    ...
    if (status === "expirado")
        return <Badge variant="destructive" className="gap-1"><XCircle className="h-3 w-3" /> Expirado há {Math.abs(diasRestantes)}d</Badge>;
    if (status === "expira_breve")
        return <Badge className="gap-1 bg-amber-500 hover:bg-amber-500/90 text-white"><AlertTriangle className="h-3 w-3" /> Expira em {diasRestantes}d</Badge>;
    return <Badge className="gap-1 bg-emerald-600 hover:bg-emerald-600/90 text-white"><CheckCircle2 className="h-3 w-3" /> Activo ({diasRestantes}d)</Badge>;
}
...
export default function ContratosInstituicao() {
    ...
    const [contratoEmEdicao, setContratoEmEdicao] = useState<InstitutionalContract | null>(null);
    ...
    const { data: responseContratos, isLoading, isFetching, error } = useQueryContracts({
        ...
        <div className="p-6 space-y-6">
            <Breadcrumb>
                <BreadcrumbList>
                    ...
                    <BreadcrumbSeparator />
                    <BreadcrumbItem><BreadcrumbLink>Crédito Educacional</BreadcrumbLink></BreadcrumbItem>
                    <BreadcrumbSeparator />
                    <BreadcrumbItem><BreadcrumbPage>Contratos da Instituição</BreadcrumbPage></BreadcrumbItem>
                </BreadcrumbList>
            </Breadcrumb>

            <div className="flex flex-col gap-2 md:flex-row md:items-end md:justify-between">
                <div>
                    <h1 className="text-3xl font-bold tracking-tight">Contratos da Instituição</h1>
                    <p className="text-muted-foreground mt-1">Consulte contratos por instituição, bolsas e monitorize expirações.</p>
                </div>
                <Button className="gap-2" onClick={openCreateModal}><Plus className="h-4 w-4" /> Novo Contrato</Button>
            </div>
                    ...
                    <div className="grid gap-4 md:grid-cols-4">
                        ...
                        <div className="space-y-3 md:col-span-2">
                            ...
                            <div className="relative">
                                ...
                                <Input className="pl-9" placeholder="Nº do contrato" value={busca} onChange={(e) => setBusca(e.target.value)} />
                            </div>
                        </div>
            ...
            {error && (
                <Alert variant="destructive">
                    ...
                    <AlertDescription>Não foi possível obter a lista de contratos. Tenta novamente mais tarde.</AlertDescription>
                </Alert>
            )}
            <div className="grid gap-4 md:grid-cols-4">
                ...
                <Card><CardContent className="p-4 flex items-center justify-between">
                    <div><p className="text-sm text-muted-foreground">Expirados</p><p className="text-2xl font-bold">{alertas?.expirados}</p></div>
            ...
            {alertas?.expirados > 0 && (
                <Alert variant="destructive">
                    ...
                    <AlertDescription>
                        {alertas?.expirados} contrato(s) nesta página estão expirados ou prestes a expirar. Renove-os para evitar interrupções nos pagamentos aos bolseiros.
                    </AlertDescription>
                </Alert>
            )}
            ...
            <Card>
                ...
                <CardContent>
                    <Table>
                        ...
                        <TableBody>
                            {isLoading ? (
                                <TableRow><TableCell colSpan={instituicaoId === "all" ? 9 : 8} className="text-center py-8 text-muted-foreground">A carregar contratos...</TableCell></TableRow>
                            ) : lista.length === 0 ? (
                                <TableRow><TableCell colSpan={instituicaoId === "all" ? 9 : 8} className="text-center py-8 text-muted-foreground">Nenhum contrato encontrado</TableCell></TableRow>
                            ) : lista.map((c) => {
                                const total = c.bolsas.reduce((s, b) => s + (b.valorDesconto ?? 0), 0);
                                const isOpen = !!expanded[c.codigoContrato];
                                const { status } = getStatus(c.dataFim);
                                const rowClassName = [
                                    "cursor-pointer",
                                    status === "expirado" && "bg-destructive/10 hover:bg-destructive/15",
                                    status === "expira_breve" && "bg-amber-500/10 hover:bg-amber-500/15",
                                ].filter(Boolean).join(" ");
                                return (
                                    <Fragment key={c.codigoContrato}>
                                        <TableRow className={rowClassName} onClick={() => setExpanded((p) => ({ ...p, [c.codigoContrato]: !p[c.codigoContrato] }))}>
                                            <TableCell>{isOpen ? <ChevronDown className="h-4 w-4" /> : <ChevronRight className="h-4 w-4" />}</TableCell>
                                            <TableCell className="font-mono">{c.codigoContrato}</TableCell>
                                            {instituicaoId === "all" && <TableCell className="flex items-center gap-2"><Building2 className="h-3.5 w-3.5 text-muted-foreground" />{c.instituicao}</TableCell>}
                                            <TableCell>{fmtDate(c.dataInicio)}</TableCell>
                                            <TableCell>{fmtDate(c.dataFim)}</TableCell>
                                            <TableCell><Badge variant="secondary">{c.bolsas.length}</Badge></TableCell>
                                            <TableCell>{fmt(total)}</TableCell>
                                            <TableCell><StatusBadge dataFim={c.dataFim} /></TableCell>
                                        ...
                                        {isOpen && (
                                            <TableRow className={rowClassName}>
                                                <TableCell colSpan={instituicaoId === "all" ? 9 : 8} className={
                                                    status === "expirado" ? "bg-destructive/5" :
                                                        status === "expira_breve" ? "bg-amber-500/5" :
                                                            "bg-muted/30"
                                                }>
                                                    <div className="p-2">
                                                        <p className="text-xs font-semibold text-muted-foreground mb-2 uppercase tracking-wide">Bolsas do contrato</p>
                                                            ...
                                                            <Table>
                                                                <TableHeader>
                                                                    <TableRow>
                                                                        ...
                                                                        <TableHead className="text-right">Nº Máx. Estudantes</TableHead>
                                                                ...
                                                                <TableBody>
                                                                    {c.bolsas.map((b) => (
                                                                        <TableRow key={b.codigoItem}>
                                                                            ...
                                                                            <TableCell><Badge variant="outline">{b.tipoCredito}</Badge></TableCell>
                                                                            <TableCell><Badge variant="outline">{b.tipoDesconto}</Badge></TableCell>
                                                                            <TableCell className="text-right">{b.numeroMaximoEstudante}</TableCell>
                                                                            <TableCell className="text-right">{formatDesconto(b.valorDesconto, b.tipoDesconto)}</TableCell>
                                                                        </TableRow>
                                                                    ))}
                                                                </TableBody>
                                                            </Table>
                    ...
                    <div className="flex flex-col gap-3 pt-4 md:flex-row md:items-center md:justify-between">
                        <div className="flex items-center gap-2 text-sm text-muted-foreground">
                            <span>
                                {total === 0 || isFetching
                                    ? isFetching ? "A actualizar..." : "Nenhum resultado"
                        ...
                        <div className="flex items-center gap-1">
                            ...
                            <Button variant="outline" size="icon" className="h-8 w-8"
                                onClick={() => setPage((p) => Math.min(totalPaginas, p + 1))} disabled={page >= totalPaginas}>
                                <ChevronRight className="h-4 w-4" />
                            </Button>
                            <Button variant="outline" size="icon" className="h-8 w-8"
                                onClick={() => setPage(totalPaginas)} disabled={page >= totalPaginas}>
            ...
            <ContratoModal
                ...
                defaultInstituicaoId={instituicaoId !== "all" ? instituicaoId : undefined}
            />
        </div>
    );
}