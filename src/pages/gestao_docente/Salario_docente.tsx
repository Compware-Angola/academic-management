...
export default function SalarioDocente() {
  ...
  const {
    ...
    isLoading,
    refetch,
  } = useQueryEstatisticaAssiduidadeDocente({
  ...
  const resumo = useMemo(
    () => ({
      totalDocentes: total,
      totalHorasEfetivas: data.reduce(
        (s, d) => s + (Number(d.total_horas_efetivas) || 0),
        0,
      ),
      totalFaltas: data.reduce(
        (s, d) => s + (Number(d.total_faltas_geral) || 0),
        0,
      ),
    ...
    <div className="space-y-6">
      <Breadcrumb>
        <BreadcrumbList>
          ...
          <BreadcrumbSeparator />
          <BreadcrumbItem>
            <BreadcrumbLink href="/gestao-docentes">
              Gestão de Docentes
            </BreadcrumbLink>
          </BreadcrumbItem>
          <BreadcrumbSeparator />
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Controle de horas, assiduidade e cálculo salarial dos docentes
          </p>
        </div>
            ...
            <div className="flex items-center justify-between rounded-lg border p-3">
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  Ignora faltas no cálculo salarial
                </p>
              </div>
              <Switch
                checked={filters.naoCobrarFaltas}
                onCheckedChange={(c) => updateFilters({ naoCobrarFaltas: c })}
              />
            </div>
            <div className="flex items-center justify-between rounded-lg border p-3">
              <div>
                <Label className="text-sm font-medium">
                  Exigir Presenças Confirmadas Pelos Sumaristas
                </Label>
                <p className="text-xs text-muted-foreground">
                  Só conta presenças confirmadas por sumaristas
                </p>
              </div>
              <Switch
                checked={filters.exigirPresencasConfirmadas}
                onCheckedChange={(c) =>
                  updateFilters({ exigirPresencasConfirmadas: c })
                }
              />
            </div>
            <div className="flex items-center justify-between rounded-lg border p-3">
              <div>
                <Label className="text-sm font-medium">
                  Exigir Sumários Inseridos
                </Label>
                <p className="text-xs text-muted-foreground">
                  Requer sumários inseridos para contabilizar
                </p>
              </div>
              <Switch
                checked={filters.exigirSumariosInseridos}
                onCheckedChange={(c) =>
                  updateFilters({ exigirSumariosInseridos: c })
                }
              />
            </div>
            <div className="flex items-center justify-between rounded-lg border p-3">
              <div>
                <Label className="text-sm font-medium">
                  Exigir Sumários Válidos
                </Label>
                <p className="text-xs text-muted-foreground">
                  Apenas sumários validados são contabilizados
                </p>
              </div>
              <Switch
                checked={filters.exigirSumariosValidos}
                onCheckedChange={(c) =>
                  updateFilters({ exigirSumariosValidos: c })
                }
              />
            </div>
          </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
        <Card className="p-3 text-center">
          ...
          <p className="text-2xl font-bold text-emerald-600">
            {Math.round(resumo.totalHorasEfetivas)}
          </p>
        </Card>
        <Card className="p-3 text-center">
            ...
            <Table>
              <TableHeader>
                ...
                <TableRow>
                  <TableHead
                    rowSpan={3}
                    className="border border-border align-middle whitespace-nowrap text-xs px-2"
                  >
                  ...
                  <TableHead
                    ...
                    className="border border-border align-middle whitespace-nowrap text-xs px-2"
                  >
                    <SortHeader field="grau_academico">
                      Grau Académico
                    </SortHeader>
                  </TableHead>
                  <TableHead
                    rowSpan={3}
                    className="border border-border align-middle text-center whitespace-nowrap text-xs px-2"
                  >
                  ...
                  <TableHead
                    rowSpan={3}
                    className="border border-border align-middle text-center whitespace-nowrap text-xs px-2 bg-blue-50 dark:bg-blue-950/30"
                  >
                    <ThTooltip
                      label="TM"
                      description="Total Mensal — total de aulas no mês actual"
                    />
                  </TableHead>
                  <TableHead
                    rowSpan={3}
                  ...
                  >
                    <ThTooltip
                      label="TS"
                      description="Total Semestral — total de aulas no semestre filtrado"
                    />
                  </TableHead>
                  <TableHead
                    rowSpan={3}
                  ...
                  >
                    <ThTooltip
                      label="TA"
                      description="Total Anual — total de aulas no ano lectivo"
                    />
                  </TableHead>

                  <TableHead
                    rowSpan={3}
                    className="border border-border align-middle text-center whitespace-nowrap text-xs px-2 bg-emerald-50 dark:bg-emerald-950/20"
                  >
                  ...
                  <TableHead
                    rowSpan={3}
                    className="border border-border align-middle text-center whitespace-nowrap text-xs px-2 bg-red-50 dark:bg-red-950/20"
                  >
                    Total Faltas
                  </TableHead>
                  ...
                  <TableHead
                    colSpan={3}
                    className="border border-border text-center text-xs font-semibold bg-emerald-100 dark:bg-emerald-900/40"
                  >
                    <ThTooltip
                      label="AP"
                      description="Aulas Presenciais — aulas realizadas em modo presencial"
                    />
                  </TableHead>

                  <TableHead
                    colSpan={3}
                    className="border border-border text-center text-xs font-semibold bg-teal-100 dark:bg-teal-900/40"
                  >
                    <ThTooltip
                      label="AV"
                      description="Aulas Virtuais — aulas realizadas em modo virtual / online"
                    />
                  </TableHead>

                  <TableHead
                    rowSpan={3}
                    className="border border-border align-middle text-center whitespace-nowrap text-xs px-2 bg-purple-50 dark:bg-purple-950/20"
                  >
                    Total Presenças
                  </TableHead>
                  <TableHead
                  ...
                  <TableHead
                    rowSpan={3}
                    className="border border-border align-middle text-center whitespace-nowrap text-xs px-2 bg-purple-50 dark:bg-purple-950/20 font-bold"
                  >
                    Total Geral
                  </TableHead>
                </TableRow>
                ...
                <TableRow>
                  <TableHead className="border border-border text-center text-xs bg-emerald-100/70 dark:bg-emerald-900/30 whitespace-nowrap px-2">
                    <ThTooltip
                      label="P"
                      description="Presenças — aulas presenciais em que o docente esteve presente"
                    />
                  </TableHead>
                  <TableHead className="border border-border text-center text-xs bg-emerald-100/70 dark:bg-emerald-900/30 whitespace-nowrap px-2">
                    <ThTooltip
                      label="F"
                      description="Faltas — aulas presenciais em que o docente faltou"
                    />
                  </TableHead>
                  ...
                  <TableHead className="border border-border text-center text-xs bg-teal-100/70 dark:bg-teal-900/30 whitespace-nowrap px-2">
                    <ThTooltip
                      label="P"
                      description="Presenças — aulas virtuais em que o docente esteve presente"
                    />
                  </TableHead>
                  <TableHead className="border border-border text-center text-xs bg-teal-100/70 dark:bg-teal-900/30 whitespace-nowrap px-2">
                    <ThTooltip
                      label="F"
                      description="Faltas — aulas virtuais em que o docente faltou"
                    />
                  </TableHead>
                ...
                {sortedData.length === 0 ? (
                  ...
                  sortedData.map((item, idx) => (
                    <TableRow key={idx} className="hover:bg-muted/50">
                      <TableCell className="border border-border font-mono text-xs whitespace-nowrap px-2">
                        {item.n_mecanografico}
                      </TableCell>
                      <TableCell className="border border-border text-xs font-medium whitespace-nowrap px-2">
                        {item.nome}
                      </TableCell>
                      <TableCell className="border border-border text-xs whitespace-nowrap px-2">
                        {item.grau_academico}
                      </TableCell>
                      <TableCell className="border border-border text-xs text-center px-2">
                        {item.escalao}
                      </TableCell>
                      <TableCell className="border border-border text-xs text-center px-2">
                        {item.categoria}
                      ...
                      <TableCell className="border border-border text-center text-xs px-2">
                        {n(item.aulas_semanais)}
                      </TableCell>
                      <TableCell className="border border-border text-center text-xs px-2">
                        {n(item.aulas_mensais)}
                      </TableCell>
                      <TableCell className="border border-border text-center text-xs px-2">
                        {n(item.total_aulas_previstas)}
                      </TableCell>

                      <TableCell className="border border-border text-center text-xs px-2 bg-blue-50/50 dark:bg-blue-950/10">
                        {n(item.tm)}
                      </TableCell>
                      <TableCell className="border border-border text-center text-xs px-2 bg-blue-50/50 dark:bg-blue-950/10">
                        {n(item.ts)}
                      ...
                      <TableCell className="border border-border text-center text-xs px-2 bg-emerald-50/40 dark:bg-emerald-950/10 text-emerald-700">
                        {n(item.ap_presenca)}
                      </TableCell>
                      <TableCell className="border border-border text-center text-xs px-2 bg-emerald-50/40 dark:bg-emerald-950/10 text-red-600">
                        {n(item.ap_falta)}
                      </TableCell>
                      <TableCell className="border border-border text-center text-xs px-2 bg-emerald-50/40 dark:bg-emerald-950/10 font-semibold">
                        {n(item.ap_total)}
                      </TableCell>

                      <TableCell className="border border-border text-center text-xs px-2 bg-teal-50/40 dark:bg-teal-950/10 text-teal-700">
                        {n(item.av_presenca)}
                      </TableCell>
                      <TableCell className="border border-border text-center text-xs px-2 bg-teal-50/40 dark:bg-teal-950/10 text-red-600">
                        {n(item.av_falta)}
                      </TableCell>
                      <TableCell className="border border-border text-center text-xs px-2 bg-teal-50/40 dark:bg-teal-950/10 font-semibold">
                        {n(item.av_total)}
                      </TableCell>

                      <TableCell className="border border-border text-center text-xs px-2 bg-purple-50/50 dark:bg-purple-950/10 text-emerald-700 font-semibold">
                        {n(item.total_presencas)}
                      </TableCell>
