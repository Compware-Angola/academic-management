...
export function ParametersEditModal({
  ...
  useEffect(() => {
    ...
    if (inicio1 && fim2) {
      ...
      setPeriodosForm(prev => ({ ...prev, designacao: `${anoInicio}/${anoFinal}` }));
    } else {
        ...
        <div className="min-h-96">
          {currentStep === "periodos" && (
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                {["Início 1º Semestre", "Fim 1º Semestre", "Início 2º Semestre", "Fim 2º Semestre"].map((label, i) => {
                  const keys = ["dataInicioPrimeiroSemestre", "dataFimPrimeiroSemestre", "dataInicioSegundoSemestre", "dataFimSegundoSemestre"] as const;
                  return (
                    <div key={label} className="space-y-2">
                      ...
                      <Input
                        ...
                        onChange={e => setPeriodosForm(prev => ({ ...prev, [keys[i]]: e.target.value }))}
                      />
                    </div>
                  );
                })}
              ...
              {loadingVagas ? (
                ...
                <div className="space-y-4 max-h-96 overflow-y-auto">
                  {vagasEditadas.map((vaga, i) => (
                    <div key={`${vaga.codigoCurso}-${vaga.codigo_periodo}`} className="flex items-center justify-between p-4 border rounded-lg bg-card">
                      <div>
          ...
          {currentStep === "mensalidades" && (
            <div className="flex flex-col items-center justify-center h-96 text-muted-foreground space-y-4">
              ...
              <p className="text-lg">Pronto! Agora é só clicar em "Concluir" para salvar tudo.</p>
            </div>
          )}
        </div>
