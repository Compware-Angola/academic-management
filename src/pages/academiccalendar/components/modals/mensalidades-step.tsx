...
type typeCandidate = "Licenciatura" | "Pos-Graduacao";
type MensalidadesStepProps = {
  tipoCandidatura: typeCandidate;
...
export function MensalidadesStep(props: MensalidadesStepProps) {
  ...
  const generateMesTempByCandidateType = (mensalidadesEditadas) => {
    return tipoCandidatura === "Licenciatura"
      ? mensalidadesEditadas.filter((item) => item.activo === 1)
      : mensalidadesEditadas.filter((item) => item.activo_posgraduacao === 1);
  };
      ...
      {loadingMeses ? (
        ...
        <div className="bg-card rounded-lg border overflow-hidden">
          <Table>
            ...
            <TableBody>
              {generateMesTempByCandidateType(mensalidadesEditadas).map(
                (item, index) => (
                  <TableRow key={index}>
                    ...
                    <TableCell>
                      {tipoCandidatura === "Licenciatura" ? (
                        <>{item.activo === 1 ? "Activo" : "Inactivo"}</>
                      ) : (
                        <>
                          {item.activo_posgraduacao === 1
                            ? "Activo"
                            : "Inactivo"}
                        </>
                      )}
