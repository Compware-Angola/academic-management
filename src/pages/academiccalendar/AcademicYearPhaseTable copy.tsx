...
// TODO: ajustar conforme o contexto real (prop, contexto global, etc.)
const TIPO_CANDIDATURA = 1;
...
export default function AcademicYearPhase() {
  ...
  const anos: AnoLetivo[] = useMemo(() => {
    return (data?.data ?? []).map((a) => ({
      id: String(a.codigo),
      nome: a.designacao,
      // fase_anolectivo: RASCUNHO / CONFIGURAVEL / USAVEL / ACTIVO / ENCERRADO
  ...
  // seleciona o primeiro ano assim que os dados chegam
  useEffect(() => {
    if (!selectedId && anos.length > 0) {
      setSelectedId(anos.find((a) => a.activo)?.id ?? anos[0].id);
    }
  }, [anos, selectedId, data]);
          ...
          <CardContent>
            ...
            {anoAtual.estado === "ACTIVO" && (
              <Alert className="mt-4">
                ...
                <AlertDescription>
                  Este é o Ano Lectivo actualmente activo. Todas as operações
                  estão disponíveis.
                </AlertDescription>
              </Alert>
            )}
