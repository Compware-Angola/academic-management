...
export function CartaConclusaoSection({ codigoMatricula }: Props) {
  ...
  const podeGerarCarta =
    studentClassInfo?.data_conclusao && studentClassInfo?.nota_obtida;
          ...
          (isLoadingClassInfo ? (
            ...
            podeGerarCarta ? (
              <GerarCartaConclusao
                ...
                cargoReitor="Reitor"
                nomeReitor={reitor?.utilizadorNome ?? "Tiago Caungo Mutombo"}
              ...
              <div className="flex items-center gap-2 text-sm text-destructive px-4 py-2 rounded-lg border border-destructive/30 bg-destructive/5">
                Não é possível gerar a carta: falta data de conclusão ou nota
                final.
              </div>
            )
          ) : (
