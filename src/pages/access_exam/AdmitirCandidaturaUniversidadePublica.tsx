...
function CandidatoDocumentoUniversidadePublica({
  candidato,
}: {
  candidato: any;
}) {
  const { mutate: validarDocumento, isPending } =
    useValidarDocumentoUniversidadePublica();

  if (candidato.tentou_universidade_publica !== 1) return null;
  ...
  return (
    <div className="mt-6 pt-4 border-t">
      <div className="flex items-center justify-between rounded-lg border bg-muted/30 p-4">
        <div>
          <p className="font-medium text-sm">Documento Universidade Pública</p>
          <p className="text-xs text-muted-foreground">
            {!documentoUniversidadePublica
              ? "Nenhum documento submetido"
              : jaValidado
                ? "Documento validado"
                : "Documento submetido, aguarda validação"}
          </p>
        </div>
        ...
        <div className="flex gap-2">
          <Button
            variant="outline"
            size="sm"
            disabled={!documentoUniversidadePublica}
          ...
          {!jaValidado && (
            <Button
              size="sm"
              disabled={!documentoUniversidadePublica || isPending}
...
export default function AdmitirCandidaturaUniversidadePublica() {
  ...
  const { mutate: admitirCandidato, isPending: isSaving } =
    useAdmitirCandidato();
  ...
  function handlePrepararNota() {
    ...
    if (!notaExame || isNaN(n) || n < 0 || n > 20) {
      toast({
        title: "Erro",
        description: "Insira uma nota válida entre 0 e 20.",
        variant: "destructive",
      });
      return;
    }
    ...
    <div className="space-y-6">
      ...
      <Breadcrumb>
        <BreadcrumbList>
          ...
          <BreadcrumbSeparator />
          <BreadcrumbItem>
            <BreadcrumbPage>Admitir Candidatura — Univ. Pública</BreadcrumbPage>
          </BreadcrumbItem>
        </BreadcrumbList>
      </Breadcrumb>
      ...
      <div>
        <h1 className="text-3xl font-bold tracking-tight">
          Admitir Candidatura — Universidade Pública
        </h1>
          ...
          <Card>
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                  <Label className="text-muted-foreground">
                    Testou em Universidade Pública?
                  </Label>
                  <p>
                    {candidato.tentou_universidade_publica == 1 ? "Sim" : "Não"}
                  </p>
                </div>
              </div>

              <CandidatoDocumentoUniversidadePublica candidato={candidato} />
            </CardContent>
          </Card>

          {/* Nota do Exame — desaparece após preparar */}
          {fase === "idle" &&
            (candidato.tentou_universidade_publica !== 1 ||
              candidato.doc_universidade_valido === 1) && (
              <Card>
                <CardHeader>
                  <CardTitle>Nota do Exame</CardTitle>
                </CardHeader>
                <CardContent>
            ...
            <Card className="border-green-200 bg-green-50/40">
              ...
              <CardContent className="space-y-4">
                <p className="text-sm text-muted-foreground">
                  ...
                  <strong>{candidato.nome}</strong>. Confirme abaixo para
                  admitir oficialmente ao curso de{" "}
