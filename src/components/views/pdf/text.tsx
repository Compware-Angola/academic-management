...
export function PresenceListPDFDocument(props: GenericPDFProps) {
  ...
  const infoSections = [
    {
      ...
      content: [
        ...
        `Curso: ${curso}`,
        turma ? `Turma: ${turma}` : "",
        unidadeCurricular ? `Unidade Curricular: ${unidadeCurricular}` : "",
        tipoAvaliacao ? `Tipo de Avaliação: ${tipoAvaliacao}` : "",
      ].filter(Boolean),
    },
    {
      title: "Resumo",
      content: [`Total de estudantes: ${total}`, filtrosExtras || ""].filter(
        Boolean,
      ),
    },
  ];
  const mainTable = {
    ...
    headers: [
      ...
      { key: "assinatura", label: "Assinatura", width: "20%" },
      { key: "nota", label: "Nota", width: "12%", align: "center" },
    ],
    rows,
  };
  const color = "#0D1B48";
  ...
  return (
    <Document>
      <Page size="A4" style={dynamicStyles.page} orientation="landscape">
