      ...
      <Page size="A4" style={S.page} orientation="portrait">
        ...
        <View style={{ ...S.header, borderBottomColor: color }}>
          ...
          <View style={S.headerRight}>
            ...
            {header.decree && <Text style={S.orgDecree}>{header.decree}</Text>}
        ...
        <Text style={S.receiptParagraph}>
          Recebi de <Text style={S.bold}>{data.studentName}</Text> BI{" "}
          <Text style={S.bold}>{data.studentId ?? "-----"}</Text> curso de{" "}
          {data.program} de <Text style={S.bold}>{data.course}</Text> a quantia
          de <Text style={S.bold}>{data.totalInWords}</Text> referente a
          pagamento à <Text style={S.bold}>{header.name}</Text> de:
        </Text>
        ...
        <View style={S.tableContainer}>
          ...
          <View style={S.tableHeaderRow}>
            <Text style={[S.tableHeaderCell, { width: COL_DATE }]}>
              Data de Pag.
            </Text>
            <Text style={[S.tableHeaderCell, { width: COL_DESC }]}>
              Descrição
            </Text>
            <Text style={[S.tableHeaderCell, { width: COL_MODE }]}>
              Modo Pag.
            </Text>
            <Text
              style={[
                S.tableHeaderCell,
                { width: COL_VALUE, borderRightWidth: 0 },
              ]}
            >
              Valor
            </Text>
          ...
          {data.payments.map((row, i) => {
            ...
            return (
              <View key={i} style={RowStyle}>
                <Text style={[S.tableCell, { width: COL_DATE }]}>
                  {row.date}
                </Text>
                <Text style={[S.tableCell, { width: COL_DESC }]}>
                  {row.description}
                </Text>
                <Text
                  style={[
                    S.tableCell,
                    { width: COL_MODE, textAlign: "center" as const },
                  ]}
                >
                  {row.paymentMode}
                </Text>
