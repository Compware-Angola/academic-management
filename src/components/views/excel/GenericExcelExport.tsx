...
// ──────────────────────────────────────────────
// Geração do workbook
export async function generateGenericExcel(props: GenericExcelProps): Promise<Blob> {
  const {
  ...
  if (header.logoSrc) {
    ...
    if (imgBuffer) {
      ...
      ws.addImage(imageId, {
        tl: { col: 0,         row: NAME_ROW - 1,  nativeCol: 0, nativeColOff: 0, nativeRow: NAME_ROW - 1, nativeRowOff: 0 },
        br: { col: LOGO_COLS, row: headerLastRow, nativeCol: LOGO_COLS, nativeColOff: 0, nativeRow: headerLastRow, nativeRowOff: 0 },
        editAs: 'oneCell',
      } as any);
    }
  }
  ...
  infoSections.forEach(section => {
    if (section.title) {
      addRow(section.title, 18, {
        font:      baseFont({ size: 12, bold: true, color: { argb: argb(PRIMARY) } }),
